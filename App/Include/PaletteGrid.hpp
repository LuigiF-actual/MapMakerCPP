...
public:
    ...
    Tile* getSelectedCell()
    {
        ...
        if (newSelect != nullptr)
        {
            ...
            if ((static_cast<int>(newSelect->scRec.x) > (m_Texture->width - Config::paletteTilesSize)) || (static_cast<int>(newSelect->scRec.y) > (m_Texture->height - Config::paletteTilesSize)))
            {
